...
dictionary <- read.csv(here::here("metadata", "MAPS_Dictionary_v2.7.csv"))
...
Find_ID_3 <- function(dictionary, ID_2){
  other_ID2s <- dictionary[dictionary$ID_2 == ID_2,] #narrows down the dictionary to all entries which ave the same ID_2
  ...
  ID3_nums <- c(ID3_nums, 0) #Adds a 0 to the list - has no effect if there are other numbers present, but if this is the first time this ID-2 is being used then it creates a number to use for the next step
  ...
  new_ID_3 <- paste0(ID2, ".", next_ID3) #the new ID_3 is created from the calculated next ID_3 sequential number, and the ID_2 input
  return(new_ID_3)
}
...
New_Dict_Entry <- function(dictionary, ID_2, FoodName_3, scientific_name, FoodEx2_code, Description1 = "", Desc1.ref = "", Description2 = "", Desc1.ref2 = ""){
  ...
  new_ID_3 <- Find_ID_3(dictionary = dictionary, ID_2 = ID_2)
